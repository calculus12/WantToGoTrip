...
//Scale linear values to the clipping planes for orthographic projection
...
{
	return eyeDepth - Linear01Depth(z, _ZBufferParams) * DEPTH_SCALAR; //1=orthographic
}
...
float3 SampleOpaqueTexture(float4 screenPos, half vFace)
{
	...
	sceneColor.r = lerp(sceneColor.r, SampleSceneColor(screenPos.xy + float2((_ScreenParams.z - 1.0), 0)).r, vFace);
	sceneColor.b = lerp(sceneColor.b, SampleSceneColor(screenPos.xy - float2((_ScreenParams.z - 1.0), 0)).b, vFace);
	#endif
